...
const SigninForm = () => {
  ...
  const [loginWithEmailPending, startLoginWithEmailPending] = useTransition();
  ...
  const onSubmit = async (e) => {
    ...
    startLoginWithEmailPending(async () => {
      await authClient.signIn.email({
        ...
        fetchOptions: {
          ...
          onError: (ctx) => {
            toast.danger(ctx.error.message || 'Sign In Failed!');
          },
        },
      });
    });
    ...
    startLoginWithGooglePending(async () => {
      await authClient.signIn.social({
        ...
        fetchOptions: {
          ...
          onError: (ctx) => {
            toast.danger(ctx.error.message || 'Sign In With Google Failed!');
          },
        },
      });
    });
  ...
  const formPending = loginWithEmailPending || loginWithGooglePending;
  ...
  return (
    ...
    >
      ...
      <ButtonBlack type="submit" className="w-full capitalize mt-3">
        {loginWithEmailPending ? <Loading text="Processing..." /> : 'Sign in'}
      </ButtonBlack>
      ...
      <Description className='flex items-center gap-2 justify-center'>
        New to the collection?
        <Link href="/auth/signup" className="text-black hover:underline">Create an Account</Link>
      </Description>
    </Form>
  );
};
